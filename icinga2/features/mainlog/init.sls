...
def run():
  ...
  config[icinga2['conf_dir'] + '/features-available/mainlog.conf'] = {
    'file.managed': [
      {'user': 'root'},
      {'group': 'root'},
      {'mode': 644},
      {'contents': utils.icinga2_object(
        {
          ...
          "attrs": icinga2['features']['mainlog'],
        },
  ...
  config['icinga2_mainlog_enable'] = {
    'cmd.run': [
      {'name': 'icinga2 feature enable mainlog'},
      {'watch_in': [
        {'service': 'icinga2_service'}
      ]},
      {'unless': 'icinga2 feature list | grep Enabled | grep -w mainlog'}
    ]
  }
